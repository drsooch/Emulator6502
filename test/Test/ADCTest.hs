...
adderTriplets =
    [ (acc, addend, carry) | acc <- [0x0 .. 0xFF], addend <- [0x0 .. 0xFF], carry <- [0x0, 0x1] ]
...
adcCarry = testCase "Carry Testing for ADC" $ do
    assertBool "Checking Carry bits"
        $ foldl' (\b (acc, add, car) -> b && addAndCheckCF acc add car) True adderTriplets
...
addAndCheckCF acc addend carry =
    ...
    in  -- Since we only deal with bytes we will lose any upper bits
        -- The conversion here is what isCarryAdd could do
        -- but the current implementation is much cleaner
        if isCarryAdd acc (acc + addend + carry) carry then result > 255 else result <= 255
